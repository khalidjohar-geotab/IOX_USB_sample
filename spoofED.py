...
if len(readback) == 6 and readback[1] == 1:
    ...
    handshakeResponse = createMessage(
        bytes([0x81, 4, deviceID % 256, (deviceID >> 8) % 256, 0, 0]))
    ...
    # data value of 10 will show on MyGeotab (because of the conversion factor of 0.1 and offset of -10)
    dataValue = 200
    dataMessage = createMessage(bytes([0x80, 6, statusDataID % 256, (
        statusDataID >> 8) % 256, dataValue % 256, (dataValue >> 8) % 256, 0, 0]))
